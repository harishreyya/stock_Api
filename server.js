...
app.post('/trades', (req, res) => {
    ...
    if (!newTrade.type || !newTrade.user_id || !newTrade.symbol || !newTrade.shares || !newTrade.price) {
        return res.status(400).send('unsucess post call');
    }
    if (newTrade.shares < 10 || newTrade.shares > 30){

        return res.status(400).send('Shares only from 10 and 30');
    }
...
app.patch('/trades/:id', (req, res) => {
    ...
    if (!newPrice) {
        return res.status(400).send('unsucess id of data');
    }
    const trades = readTrades();
    const trade = trades.find(t => t.id === tradeId);
    if (!trade) {
        return res.status(404).send('id not there');
    }
...
app.delete('/trades/:id', (req, res) => {
    ...
    writeTrades(trades);
    res.status(200).send('deleted done');
});
